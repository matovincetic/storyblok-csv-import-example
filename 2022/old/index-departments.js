...
const config = {
  ...
  parentFolder: process.env.FOLDERID_DEPARTMENTS // navigate into your folder and copy the id from the URL at app.storyblok.com <- last one 
}

let stream = require('./departments.json')
...
stream.stories.forEach(storyFromJson => {
  let story = {
    ...
    content: {
      name: `${storyFromJson.name}`,
      link: storyFromJson.content.link ? storyFromJson.content.link : '',
      part_of_institute: {
          id: storyFromJson.content.part_of_institute.id,
          linktype: "story",
          fieldtype: "multilink",
          cached_url: storyFromJson.content.part_of_institute.cached_url
      },
      component: 'data_department',
    }
  }
